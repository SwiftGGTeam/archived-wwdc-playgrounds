...
<template>
  <TabGroup 
    ...
    <TabList class="flex gap-y-10 overflow-x-auto pb-4 pl-4 sm:px-0 sm:flex-col sm:pr-8">
      <Tab as="div" class="focus:outline-none relative w-3/4 pr-4 flex-none sm:w-auto" v-for="(sche, idx) in offlineSchedule">
        <OfflineDaySummary class="text-start" :class="idx != selectedTab && 'opacity-70'" :sche="sche" />
      </Tab>
    </TabList>

    <TabPanels>
      <TabPanel v-for="sche in offlineSchedule">
        <OfflineTimeSlots :timeSlots="sche.timeSlots" />
      </TabPanel>
    </TabPanels>
  </TabGroup>
</template>